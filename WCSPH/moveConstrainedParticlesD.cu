...
__global__ void moveConstrainedParticlesD(struct particleStructure* pparticles, struct paramsType* pparams, kinematicsFunctionStructure* pkinematicsFunction) {
	...
	double newLocation;
	int storageStride = pparams->storageStride;
	...
	for (int ind1 = 0; ind1 < nFunctions; ind1++)
	{
		...
		if (index >= g1 && index <= g2)
		{
			...
			else if (xFunctionType == 2) //linear
			{
				...
				//pparticles->x[index] = newLocation;
				double disp = 0;
				for (int ind2 = 0; ind2 < 400; ind2++) {
					disp += (pparticles->Ai[ind2] / (pparticles->Bi[ind2])) * cos(pparticles->ki[ind2] * (p1x) - 2 * PI * pparticles->fi[ind2] * (t-p2x));
					//disp += (pparticles->Hi[ind2] / (2 * pparticles->Bi[ind2])) * sin(pparticles->wi[ind2] * t);
				}
				newLocation = pparticles->x[index] + disp;
				pparticles->vx[index] = (newLocation - pparticles->previousX[localIndex]) / dt;
				pparticles->x[index] = newLocation;
			...
			else if (xFunctionType == 3) //sinusoid
			{
				...
				double a = p4x;
				double eI = k0 * a0;
				double s2 = 1.414213562;
				double alpha = a0 * k0 * w0/s2;
				double beta = a0*a0*k0*k0*w0/4 + w0;
				int counter = floor(t / (storageStride * 0.0005 * 0.5));

				double w1 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues1[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(beta * w0 * t + pparticles->phaseValues1[counter]);
				double w2 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues2[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(beta * w0 * t + pparticles->phaseValues2[counter]);
				double w3 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues3[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(beta * w0 * t + pparticles->phaseValues3[counter]);
				double w4 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues4[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(beta * w0 * t + pparticles->phaseValues4[counter]);
				double w5 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues5[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(beta * w0 * t + pparticles->phaseValues5[counter]);
				double w6 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues6[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(beta * w0 * t + pparticles->phaseValues6[counter]);

				//double w1 = a0 * 1 / cosh(pparticles->phaseValues1[counter] - alpha * t) * cos(beta * w0 * t + pparticles->phaseValues1[counter]);

				//double w2 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues2[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(pparticles->phaseValues2[counter]);
				//double w3 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues3[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(pparticles->phaseValues3[counter]);
				//double w4 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues4[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(pparticles->phaseValues4[counter]);
				//double w5 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues5[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(pparticles->phaseValues5[counter]);
				//double w6 = a0 * 1 / cosh((eI / s2) * pparticles->phaseValues6[counter] - (eI * eI * eI / (4 * s2)) * w0 * t) * cos(pparticles->phaseValues6[counter]);
				
				newLocation = a * (w1 + w2 + w3 + w4 + w5 + w6);
				pparticles->vx[index] = (newLocation - pparticles->previousX[localIndex]) / dt;
				pparticles->x[index] = newLocation;
				*/
			}
			...
			*/
			...
			else if (xFunctionType == 6) //Peregrine soliton, a0, l, leadTime, targetDistance
			{
				...
				newLocation = pparticles->x[index] + pm * p5x;   //add the Peregrine modulation
				pparticles->vx[index] = (newLocation - pparticles->previousX[localIndex]) / dt;
				pparticles->x[index] = newLocation;
				...
				double2 A = { cos(-k0 * k0 * p1x * p1x * w0 * tInternal / 2.),sin(-k0 * k0 * p1x * p1x * w0 * tInternal / 2) };
				double2 B = { 4,-4 * k0 * k0 * p1x * p1x * w0 * tInternal };
				double  C = 2 * sqrt(2.0) * k0 * k0 * p1x * (p3x - Cg * tInternal);
				double  D = k0 * k0 * k0 * k0 * p1x * p1x * p1x * p1x * w0 * w0 * tInternal * tInternal;
				double  E = 1 + C * C + D;
				double2 F = { 1 - B.x / E,-B.y / E };
				double2 qp = { p1x * ((A.x * F.x) - (A.y * F.y)),p1x * ((A.x * F.y) + (F.x * A.y)) };//envelope
				double  pm = qp.x * cos(k0 * p3x - w0 * tInternal + PI) - qp.y * sin(k0 * p3x - w0 * tInternal + PI); //real part of modulated wave

				newLocation = pparticles->x[index] + pm * p4x;   //add the Peregrine modulation
				pparticles->vx[index] = (newLocation - pparticles->previousX[localIndex]) / dt;
				pparticles->x[index] = newLocation;
				*/
			}
