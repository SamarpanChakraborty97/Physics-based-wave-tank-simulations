...
__device__ void forcesInCell2(int2 neighboor, int index, double posX, double posY, double velX, double velY, double rho, double rhox, double rhoy, double pressurei, struct particleStructure* pparticles, struct paramsType* params, double* stateRates);
...
__global__ void computedVdt(struct particleStructure* pparticles, struct paramsType* pparams) {
	...
	double pressurei = computePressure(rhoi, rhoRef, tenVMaxSq);
		...
		if ((newY > -1) && (newY < (*pparams).nCellsY)) {
			...
			for (int x = -1; x <= 1; x++) {
				...
				if ((newX > -1) && (newX < (*pparams).nCellsX)) {
					...
					forcesInCell2(neighboor, index, posXi, posYi, velXi, velYi, rhoi, rhoxi, rhoyi, pressurei, pparticles, pparams, stateRates);
...
__device__ void forcesInCell2(int2 neighboor, int index, double posXi, double posYi, double velXi, double velYi, double rhoi, double rhoxi, double rhoyi, double pressurei, struct particleStructure* pparticles, struct paramsType* pparams, double* stateRates) {
		...
		for (int ind1 = startIndex; ind1 < endIndex; ind1++) {
			...
			*/
			...
			if ((rSq <= 4*(*pparams).h2) && (rSq > 0)) {  //if they are close enough, proceede
				...
				double pressurej = computePressure(rhoj, rhoRef, tenVMaxSq);
				...
				*/
				...
				double dirVel = dvxij * dx + dvyij * dy;
				double rhoBarij = (rhoi + rhoj) / 2;      //used in XSPH as well
				double Cij = sqrt(tenVMaxSq);  //mean speed of sound
				...
				stateRates[4] += term1 + pparams->delta * h * 10 * pparams->vf * term3;
