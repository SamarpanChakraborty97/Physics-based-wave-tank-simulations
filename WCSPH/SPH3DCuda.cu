...
#define TIME(processNumber,...) __VA_ARGS__;
	...
	std::cout << '\n' << params->nTotal;//number of particles
	...
	double3 globalMin = { particles->x[0],particles->y[0],particles->z[0] }; //3D
	double3 globalMax = { particles->x[0],particles->y[0],particles->z[0] }; //3D
	...
	for (int ind1 = 0; ind1 < nTotal; ind1++) {
		...
		if (particles->z[ind1] > globalMax.z) { globalMax.z = particles->z[ind1]; };
		if (particles->z[ind1] < globalMin.z) { globalMin.z = particles->z[ind1]; };
	}
	params->globalOriginX = globalMin.x;
	params->globalOriginY = globalMin.y;
	params->globalOriginZ = globalMin.z;          //3D

	std::cout << '\n' << "X-origin is:" <<params->globalOriginX;
	std::cout << '\n' << "Y-origin is:" << params->globalOriginY;
	std::cout << '\n' << "Z-origin is:" << params->globalOriginZ;

	double3 delta = { globalMax.x - globalMin.x,globalMax.y - globalMin.y,globalMax.z - globalMin.z }; //3D
	...
	params->nCellsZ = (int)(delta.z / binSize) + 1;                       //3D
	params->nCellsTotal = (params->nCellsX) * (params->nCellsY) * (params->nCellsZ); //3D
	//std::cout << '\n' << params->nCellsX;
	//std::cout << '\n' << params->nCellsY;
	//std::cout << '\n' << params->nCellsZ;
	std::cout << '\n' << "The total no. of cells is:"<<params->nCellsTotal;
	...
	double h2 = h * h;
	double h3 = h2 * h;
	//double const1 = 315.0 / (3.141592654 * 64 * h2 * h);            // 3D  //poly6 3D                - verified
	...
	//double spikyImprovedD = -15 / (3.141592654 * h2 * h2);                // 3D // Spikey better Derivative - verified
	double constWendland = 21 / (16 * 3.141592654 * h2 * h);
	double constWendlandD = -105 / (16 * 3.141592654 * h2 * h2);
	...
	//params->constPressure = constPressure;
	//params->constViscosity = constViscosity;
	params->tenVMaxSq = (10 * params->vf) * (10 * params->vf);
	params->ind1 = 0;
	//params->quartic = quartic;
	//params->quarticD = quarticD;
	//params->spikyImprovedD = spikyImprovedD;
	params->constwendland = constWendland;
	params->constwendlandD = constWendlandD;
	...
	if (params->importDensity == 0) {  //nned to initialize density
	...
	computePressure << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams);     //compute
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		cudaDeviceSynchronize();
		TIME(12, cudaMemcpy(particles->x, pdParticlesHostMirror->x, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		TIME(12, cudaMemcpy(particles->y, pdParticlesHostMirror->y, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		TIME(12, cudaMemcpy(particles->z, pdParticlesHostMirror->z, nTotal * sizeof(double), cudaMemcpyDeviceToHost););   // 3D
		TIME(12, cudaMemcpy(particles->density, pdParticlesHostMirror->density, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		//TIME(12, cudaMemcpy(particles->vy, pdParticlesHostMirror->vy, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		//TIME(12, cudaMemcpy(particles->vz, pdParticlesHostMirror->vz, nTotal * sizeof(double), cudaMemcpyDeviceToHost);); // 3D
		//TIME(12, cudaMemcpy(particles->density, pdParticlesHostMirror->density, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
#if timeComponents																		//
		cudaEventRecord(stop, 0);												//
		cudaEventSynchronize(stop);												//
...
#pragma omp section
		...
		////////////////MOVE PARTICLES, UPDATE VELOCITY, UPDATE DENISTY////////////////////
					...
					*/
					TIME(9, copySortedRhoToDensity << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
					TIME(10, updatePositionFreeParticles << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););  //have to re-bin in order to accurately reinitialize
					//move the boundary
					//we only need to cover the constrained particles
					if (numberBlocksConstrained > 0) {
						TIME(11, moveConstrainedParticlesD << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams, pdKinematicsFunction););
