...
int moveDataToGPU(particleStructure* particles, paramsType* params, std::vector<kinematicsFunctionStructure>* kinematicsFunction, struct  particleStructure** ppdParticles, struct paramsType** ppdParams, struct kinematicsFunctionStructure** ppdKinematicsFunction, particleStructure** pdParticlesHostMirror) {
	...
	//double* dPhaseValues1 = 0; cudaMalloc((void**)&dPhaseValues1, totPhases * sizeof(double)); cudaMemcpy(dPhaseValues1, particles->phaseValues1, totPhases * sizeof(double), cudaMemcpyHostToDevice);
	//double* dPhaseValues2 = 0; cudaMalloc((void**)&dPhaseValues2, totPhases * sizeof(double)); cudaMemcpy(dPhaseValues2, particles->phaseValues2, totPhases * sizeof(double), cudaMemcpyHostToDevice);
	//double* dPhaseValues3 = 0; cudaMalloc((void**)&dPhaseValues3, totPhases * sizeof(double)); cudaMemcpy(dPhaseValues3, particles->phaseValues3, totPhases * sizeof(double), cudaMemcpyHostToDevice);
	//double* dPhaseValues4 = 0; cudaMalloc((void**)&dPhaseValues4, totPhases * sizeof(double)); cudaMemcpy(dPhaseValues4, particles->phaseValues4, totPhases * sizeof(double), cudaMemcpyHostToDevice);
	//double* dPhaseValues5 = 0; cudaMalloc((void**)&dPhaseValues5, totPhases * sizeof(double)); cudaMemcpy(dPhaseValues5, particles->phaseValues5, totPhases * sizeof(double), cudaMemcpyHostToDevice);
	//double* dPhaseValues6 = 0; cudaMalloc((void**)&dPhaseValues6, totPhases * sizeof(double)); cudaMemcpy(dPhaseValues6, particles->phaseValues6, totPhases * sizeof(double), cudaMemcpyHostToDevice);
	double* dAi = 0; cudaMalloc((void**)&dAi, components * sizeof(double)); cudaMemcpy(dAi, particles->Ai, components * sizeof(double), cudaMemcpyHostToDevice);
	double* dBi = 0; cudaMalloc((void**)&dBi, components * sizeof(double)); cudaMemcpy(dBi, particles->Bi, components * sizeof(double), cudaMemcpyHostToDevice);
	double* dfi = 0; cudaMalloc((void**)&dfi, components * sizeof(double)); cudaMemcpy(dfi, particles->fi, components * sizeof(double), cudaMemcpyHostToDevice);
	double* dki = 0; cudaMalloc((void**)&dki, components * sizeof(double)); cudaMemcpy(dki, particles->ki, components * sizeof(double), cudaMemcpyHostToDevice);
	...
	//(*pdParticlesHostMirror)->phaseValues1 = dPhaseValues1;
	//(*pdParticlesHostMirror)->phaseValues2 = dPhaseValues2;
	//(*pdParticlesHostMirror)->phaseValues3 = dPhaseValues3;
	//(*pdParticlesHostMirror)->phaseValues4 = dPhaseValues4;
	//(*pdParticlesHostMirror)->phaseValues5 = dPhaseValues5;
	//(*pdParticlesHostMirror)->phaseValues6 = dPhaseValues6;
	(*pdParticlesHostMirror)->Ai = dAi;
	(*pdParticlesHostMirror)->Bi = dBi;
	(*pdParticlesHostMirror)->fi = dfi;
	(*pdParticlesHostMirror)->ki = dki;
