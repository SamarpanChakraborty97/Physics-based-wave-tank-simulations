...
__device__ double densityInCell2(int2 neighboor, int index, double posX, double posY, struct particleStructure* pparticles, struct paramsType* pparams);

__global__ void initializeDensity(struct particleStructure* pparticles, struct paramsType* pparams) {
		...
		if ((currentY > -1) && (currentY < (*pparams).nCellsY)) {
			...
			for (int x = -1; x <= 1; x++) {
				...
				if ((currentX > -1) && (currentX < (*pparams).nCellsX)) {
					...
					density += densityInCell2(neighboor, index, posXi, posYi, pparticles, pparams);
				}
			}
		}
	}

	//write new density back to original unsorted position
	//int originalIndex = dGridParticleIndex[index];
	//pRho[originalIndex] = density;

	//write the density to the sorted position to be used later in the forces routine

#if 0 //limit the denisty
	if (density > 6.0 * (*pparams).rRef)
	{
		density = 6.0 * (*pparams).rRef;
...
__device__ double densityInCell2(int2 neighboor, int index, double posXi, double posYi, struct particleStructure* pparticles, struct paramsType* pparams) {
	...
	if (startIndex != 0xffffffff) {
		...
		for (int ind1 = startIndex; ind1 < endIndex; ind1++) {
			...
			double m2 = pparams->mass;  //mass; right now these are identical  for all particles
			...
			if ((rSq <= 4*(*pparams).h2) && (rSq >= 0)) {
				...
				density += m2 * wendland((*pparams).constwendland, rOh); //   4/(pi h^2)*(1-r^2/h^2)^3
