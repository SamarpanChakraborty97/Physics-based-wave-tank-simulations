...
//__global__ void rhoRenormalize(struct particleStructure* pdParticles, struct paramsType* dParams);
//__global__ void computeMLS(struct particleStructure* pdParticles, struct paramsType* dParams);
...
#define TIME(processNumber,...) __VA_ARGS__;
	...
	//heartbeat
	cudaEvent_t tStartProcess, tStopProcess, tStartWhole, tStopWhole;
	cudaEventCreate(&tStartProcess);
	cudaEventCreate(&tStopProcess);
	cudaEventCreate(&tStartWhole);
	cudaEventCreate(&tStopWhole);

	cudaEventRecord(tStartWhole);
	cudaEventRecord(tStartProcess);

	float elapsedTimeWhole;
	float elapsedTimeProcess;
	std::cout << '\n' << omp_get_max_threads();
	...
	std::cout<<'\n'<<params->globalOriginX;
	std::cout<<'\n'<<params->globalOriginY;
	std::cout<<'\n'<<binSize;
	...
	double h = params->h;  //make particles 2x large here and above; use 2x smoothing length; revert in viscosity muij
	double h2 = h * h;
	...
	params->ind1 = 0;
	params->relaxStart = globalMax.x - 3.0;
	params->relaxEnd = globalMax.x;
	params->betaDis = 10;
	//params->quartic = quartic;
	//params->quarticD = quarticD;
	//params->spikyImprovedD = spikyImprovedD;
	//params->constDensity = const1;
	...
	//moveConstrainedParticlesD increments the GPU copy of ind1
	...
	//std::cout << "Has it proceded till here?" << std::endl;
	...
	std::cout << "The total number of simulations is: " << nT << std::endl;
	while (iteration < nT)
	{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
		cudaDeviceSynchronize();
		TIME(11, cudaMemcpy(particles->x, pdParticlesHostMirror->x, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		TIME(11, cudaMemcpy(particles->y, pdParticlesHostMirror->y, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		...
		TIME(11, cudaMemcpy(particles->density, pdParticlesHostMirror->density, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		//TIME(10, cudaMemcpy(particles->pressure, pdParticlesHostMirror->pressure, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		//TIME(17, cudaMemcpy(particles->L1, pdParticlesHostMirror->L1, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		//TIME(17, cudaMemcpy(particles->rhoGradX, pdParticlesHostMirror->rhoGradX, nTotal * sizeof(double), cudaMemcpyDeviceToHost););
		//std::cout << "Has it proceded till here?" << std::endl;
		//std::cout <<'\n' <<  params->nMeasured;
...
#pragma omp section
			{
				...
				for (int ind2 = 0; ind2 < storageStride; ind2++)
				{
					...
					*/
					...
					/////////////////COMPUTE DV/DT////////////////////////////////////////////////////////////
					//TIME(3, computeMLS << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););		//
					//TIME(4, rhoRenormalize << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););

					//std::cout << "Has it proceded till here?" << std::endl;
					TIME(3, computeKernelCorrectionTerms << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););

					//std::cout << "Has it proceded till here?" << std::endl;
					TIME(4, computedVdt << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
					
					/*
					double *tempInt = new double[nTotal];
					cudaMemcpy(tempInt, pdParticlesHostMirror->rhoGradX, nTotal*sizeof(int),cudaMemcpyDeviceToHost);
					for (int ind3 = 0;ind3<nTotal;ind3++){
						printf("%d\n",tempInt[ind3]);
					}
					*/
					//TIME(6, computedRhodt << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
		//////////////////////////////////////////////////////////////////////////////////////////


		////////////////MOVE PARTICLES, UPDATE VELOCITY, UPDATE DENISTY////////////////////
					//std::cout << "Has it proceded till here?" << std::endl;
					TIME(5, updateVelWithXSPH << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););

					//std::cout << "Has it proceded till here?" << std::endl;
					TIME(6, updateVelocity << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););

					//std::cout << "Has it proceded till here?" << std::endl;
					TIME(7, updateDensity << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););

					//std::cout << "Has it proceded till here?" << std::endl;
					//cudaMemcpy(particles->density, pdParticlesHostMirror->density, nTotal * sizeof(double), cudaMemcpyDeviceToHost);
					//printf("%d\n", particles->density);
					...
					{
						TIME(9, reinitializeDensity << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
						TIME(10, moveFilteredDensityToDensity << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
						//DEBUG(DEBUGdisplayMoveDensity << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););				   //
					}
					*/
					TIME(8, copySortedRhoToDensity << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););

					//std::cout << "Has it proceded till here?" << std::endl;
					TIME(9, updatePositionFreeParticles << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););  //have to re-bin in order to accurately reinitialize
					//std::cout << "Has it proceded till here?" << std::endl;
					
					//TIME(12, computeShift << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
					//TIME(13, computeShiftGrad << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
					//TIME(14, computeDiv << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
					//TIME(15, updateShift << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams););
					//move the boundary
					//we only need to cover the constrained particles
					if (numberBlocksConstrained > 0) {
						TIME(10, moveConstrainedParticlesD << <numberBlocks, threadsPerBlock >> > (pdParticles, pdParams, pdKinematicsFunction););
		...
		////////////////////////////////////////////////////////////////////////////////////////
					...
					{
						cudaEventRecord(tStopProcess);
						cudaEventSynchronize(tStopProcess);
						cudaEventElapsedTime(&elapsedTimeProcess, tStartProcess, tStopProcess);
						printf("%5.0d steps took %f ms; %f ms/step;OMP thread # % u;\n", storageStride, elapsedTimeProcess, elapsedTimeProcess / storageStride, omp_get_thread_num());
						//OMP thread # % u
						//omp_get_thread_num()
						cudaEventRecord(tStartProcess);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	cudaEventRecord(tStopWhole);
	cudaEventSynchronize(tStopWhole);
	cudaEventElapsedTime(&elapsedTimeWhole, tStartWhole, tStopWhole);
	printf("\nThe whole process took %f ms;\n", elapsedTimeWhole);

	std::ofstream outFile("Running_Times_GPU_CCSPH.txt");
	outFile << elapsedTimeWhole << "\n";
