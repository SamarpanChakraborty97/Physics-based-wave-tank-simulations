...
__global__ void moveConstrainedParticlesD2(struct particleStructure* pparticles, struct paramsType* pparams, kinematicsFunctionStructure* pkinematicsFunction) {
	...
	*/
	...
	for (int ind1 = 0; ind1 < nFunctions; ind1++)
	{
		...
		if (index >= g1 && index <= g2)
		{
			...
			else if (xFunctionType == 3) //sinusoid ( 2nd order wave theory)
			{
				//computing a few parameters
				double k = 2 * PI / p4x;
				double m1_denom = sinh(k * p5x) * cosh(k * p5x) + (k * p5x);
				double m1_num = 2 * (sinh(k * p5x) * sinh(k * p5x));
				double m1 = m1_num / m1_denom;
				double H = m1 * p1x;
				double a = 3 * cosh(k * p5x) / (sinh(k * p5x) * sinh(k * p5x) * sinh(k * p5x));
				double b = 2 / m1;
				//double a = (H * H) / (32 * p5x);

				//double k = 2 * PI / p4x;
				//double n1 = 0.5 * (1 + (2 * k * p5x/sinh(2 *k*p5x)));
				//double H = p1x * tanh(k * p5x) / n1;
				//double b = n1/2;
				//double a = 3/ (4 * sinh(k * p5x) * sinh(k * p5x));
